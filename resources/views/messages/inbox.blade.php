...
@section('content')
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="text-success">Inbox</h2>
            <div>
                <a href="{{ route('messages.sent') }}" class="btn btn-outline-success">Sent Messages</a>
                ...
                <tbody>
                    @foreach($messages as $message)
                        <tr class="{{ !$message->is_read ? 'fw-bold' : '' }}">
                            <td>
                                <a href="{{ route('profile.show', $message->sender) }}" class="text-success">
                                    {{ $message->sender->name }}
                                </a>
                            </td>
                            <td>
